...
export default function FuelIntakeDisplay() {
  ...
  // Handle edit record
  const handleEditRecord = async (record: FuelIntakeRecord, e: React.MouseEvent) => {
  ...
  const handleRecordUpdate = (updatedRecord: FuelIntakeRecord) => {
    // Update the record in the local state
    setRecords(prev => prev.map(record => 
      record.id === updatedRecord.id ? updatedRecord : record
    ));
    
    // Refresh records list to get the latest data
    fetchRecords();
  };
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-lg border border-gray-100">
      ...
      <div className="p-6 rounded-xl text-white relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center mb-4 relative z-10 gap-4">
          <div>
          ...
          <Button 
            ...
            className="px-4 py-2 backdrop-blur-md bg-[#F08080]/30 border border-white/20 text-white shadow-lg hover:bg-[#F08080]/40 transition-all font-medium rounded-xl flex items-center gap-2"
          >
                  ...
                  >
                    ...
                    <TableCell className="text-right">
                      {typeof record.price_per_kg === 'number' ? (
                        record.price_per_kg.toLocaleString(undefined, {minimumFractionDigits: 3, maximumFractionDigits: 3})
                      ) : (
                        <span className="text-gray-400 italic">N/A</span>
                      )}
                    </TableCell>
                      ...
                      <div className="flex justify-center space-x-2">
                        ...
                        {(authUser?.role === 'ADMIN' || authUser?.role === 'KONTROLA' || authUser?.role === 'FUEL_OPERATOR') && (
                          <Button 
                            ...
                            className="bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100 transition-colors w-8 h-8 p-0"
                            title="Uredi"
                          >
                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">

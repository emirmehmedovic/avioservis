...
// Prošireni tip za FuelingOperationFormData koji uključuje usd_exchange_rate i payment_method
...
const AddOperationForm: React.FC<AddOperationFormProps> = ({
  ...
  const handleSubmitWrapper = (e: React.FormEvent) => {
    e.preventDefault();
    setCustomError(null); // Reset error on new submission

    const selectedTank = tanks.find(tank => tank.id === parseInt(formData.tankId));
    const quantityKg = parseFloat(textInputs.quantity_kg);

    if (selectedTank && quantityKg > 0) {
      const availableKg = Number(selectedTank.current_kg || 0);
      if (quantityKg > availableKg) {
        setCustomError(`Nedovoljno goriva u cisterni. Traženo: ${quantityKg.toFixed(2)} kg, Dostupno: ${availableKg.toFixed(2)} kg.`);
        return;
      }
    }
  ...
  return (
    <div 
      ...
      onClick={(e) => {
        // Zatvori formu ako se klikne na pozadinu (ne na formu)
    ...
    >
      <form 
        ...
        className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden max-w-6xl w-full max-h-[90vh] overflow-y-auto relative"
      ...
      <button
        type="button"
        onClick={onCancel}
        className="absolute top-4 right-4 z-10 p-2 rounded-full hover:bg-white/20 transition-all duration-200 backdrop-blur-sm border border-white/20 text-gray-600 hover:text-gray-800"
      ...
      <div className="p-6 border-b border-white/30">
        {customError && (
          <div className="p-4 mb-4 text-sm text-red-800 rounded-xl bg-red-50/80 backdrop-blur-sm border border-red-200/50 shadow-lg" role="alert">
            <span className="font-medium">Greška prilikom validacije:</span> {customError}
          </div>
        )}
        ...
        <div className="space-y-8">
          ...
          <div className="bg-gradient-to-br from-slate-100/60 to-gray-100/60 backdrop-blur-lg rounded-2xl p-5 border border-white/40 shadow-xl">
              ...
              <div>
                ...
                <div className="mt-1">
                  <input
                    ...
                    onChange={(e) => {
                      // Transformiraj u velika slova i ukloni razmake
                      const transformedValue = e.target.value.toUpperCase().replace(/\s/g, '');
                      handleInputChange({
                        target: {
                          name: 'flight_number',
                    ...
                    onKeyDown={(e) => {
                      // Spriječi unos razmaka
                      if (e.key === ' ') {
                        e.preventDefault();
                      }
                    }}
                    className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300/50 rounded-lg h-11 bg-white/80 backdrop-blur-sm"
                    placeholder="npr. W44279"
                  />
                </div>
                <p className="mt-1 text-xs text-gray-500">
                  Broj leta se automatski transformira u velika slova i uklanjaju se razmaci
                </p>
              </div>
          ...
          <div className="bg-gradient-to-br from-stone-100/60 to-neutral-100/60 backdrop-blur-lg rounded-2xl p-5 border border-white/40 shadow-xl">
            <h3 className="text-md font-medium text-gray-700 mb-4 flex items-center">
              <svg className="w-5 h-5 mr-2 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2">
                  ...
                  >
                    ...
                    {tanks.map((tank) => {
                      ...
                      const tankKg = tank.current_kg !== undefined ? 
                        tank.current_kg : 
                        (tank.current_liters || 0) * 0.8;

                      return (
                        <option key={tank.id} value={tank.id} disabled={(tank.current_liters || 0) <= 0}>
                          {tank.identifier} - {tank.name} ({Number(tankKg).toFixed(1)} kg | {Number(tank.current_liters || 0).toFixed(1)} L)
                        </option>
                      );
                    })}
                  </select>
              ...
              {/* PRIORITET: Prvo prikazujemo polje za kilograme */}
              <div>
                <label htmlFor="quantity_kg" className="flex items-center text-sm font-medium text-gray-700">
                  Količina goriva (kg) <span className="ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800">Prioritet</span>
                </label>
                <div className="mt-1 relative rounded-md shadow-sm">
                  <input
                    ...
                    onChange={handleInputChange}
                    className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-indigo-700 border-2 rounded-lg pr-12 h-11 bg-white/80 backdrop-blur-sm"
                    required
                  />
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    <span className="text-indigo-700 font-medium sm:text-sm">kg</span>
                  </div>
                </div>
                <p className="mt-1 text-xs text-indigo-700">Unesite stvarnu masu goriva u kilogramima</p>
              </div>

              <div>
                <label htmlFor="quantity_liters" className="block text-sm font-medium text-gray-500">
                  Količina goriva (litara)
                </label>
                <div className="mt-1 relative rounded-md shadow-sm">
                  <input
                    type="text"
                    ...
                    onChange={handleInputChange}
                    className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300/50 rounded-lg pr-12 h-11 bg-white/80 backdrop-blur-sm"
                    required
                  />
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                ...
                <p className="mt-1 text-xs text-gray-500">Unesite količinu goriva u litrama</p>
              </div>
              ...
              <div>
                ...
                <div className="mt-1">
                  <input
                    ...
                    readOnly
                    className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300/50 rounded-lg bg-gray-50/80 backdrop-blur-sm h-11"
                    title="Automatski izračunato iz količine u kg i litrama"
                  />
                  <p className="mt-1 text-xs text-gray-500">Specifična gustoća se automatski izračunava na osnovu odnosa kg i litara</p>
                </div>
              </div>

              {/* Polje za litre sada se nalazi iznad polja za specifičnu gustoću */}
            </div>
          </div>
          {/* Currency and Price Section - Highlighted */}
          <div className="bg-gradient-to-br from-zinc-100/60 to-slate-100/60 backdrop-blur-lg rounded-2xl p-4 my-4 border border-white/40 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="mt-1 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300/50 rounded-lg h-11 bg-white/80 backdrop-blur-sm"
                  required
                >
              ...
              {formData.currency === 'USD' && (
                <div>
                  ...
                  {usdRateError && (
                    <div className="mt-2 p-2 text-sm text-red-800 rounded-xl bg-red-50/80 backdrop-blur-sm border border-red-200/50 shadow-lg" role="alert">
              ...
              {priceRuleMessage && (
                <div className="mt-3 text-sm col-span-1 md:col-span-2"> 
                  {!suggestedPrice && priceRuleMessage.includes("Nema pravila") ? (
                    <p className="text-orange-600 p-2 bg-orange-50/80 backdrop-blur-sm rounded-xl border border-orange-200/50 shadow-lg flex items-center">
                    ...
                    <p className="text-gray-600 p-2 bg-gray-100/80 backdrop-blur-sm rounded-xl border border-gray-200/50 shadow-lg flex items-center">
                    ...
                    <p className="text-green-600 p-2 bg-green-50/80 backdrop-blur-sm rounded-xl border border-green-200/50 shadow-lg flex items-center">
                    ...
                    <p className="text-blue-600 p-2 bg-blue-50/80 backdrop-blur-sm rounded-xl border border-blue-200/50 shadow-lg flex items-center">
                      <svg className="w-5 h-5 mr-2 shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path></svg>
                      {priceRuleMessage} 
                    </p>
                  )}
              ...
              {isPriceManuallySet && suggestedPrice && suggestedPrice !== textInputs.price_per_kg && (
                <div className="mt-2 col-span-1 md:col-span-2"> 
                    <button
                      ...
                      className="text-sm text-indigo-600 hover:text-indigo-800 font-medium p-2 bg-indigo-50/80 backdrop-blur-sm rounded-xl border border-indigo-200/50 hover:bg-indigo-100/80 hover:shadow-lg transition-all duration-200 w-full text-left flex items-center"
                    >
          ...
          <div className="bg-gradient-to-br from-gray-100/60 to-zinc-100/60 backdrop-blur-lg rounded-2xl p-5 border border-white/40 shadow-xl">
            <h3 className="text-md font-medium text-gray-700 mb-4 flex items-center">
              <svg className="w-5 h-5 mr-2 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 bg-indigo-50/60 backdrop-blur-sm p-4 rounded-xl border border-indigo-200/50 shadow-lg">
          ...
          <div className="bg-gradient-to-br from-neutral-100/60 to-stone-100/60 backdrop-blur-lg rounded-2xl p-5 border border-white/40 shadow-xl">
            <h3 className="text-md font-medium text-gray-700 mb-4 flex items-center">
              <svg className="w-5 h-5 mr-2 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="mt-1">
                  <select
                    ...
                    className={`shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm rounded-md ${
                      !formData.payment_method ? 'border-red-300' : 'border-gray-300'
                    }`}
                    required
                  >
              ...
              <div className="sm:col-span-2">
                ...
                <div className="mt-1">
                  <textarea
                    ...
                    onChange={handleInputChange}
                    className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300/50 rounded-lg h-20 bg-white/80 backdrop-blur-sm"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-slate-100/60 to-gray-100/60 backdrop-blur-lg rounded-2xl p-5 border border-white/40 shadow-xl">
            ...
            <div className="mt-1">
              ...
              <label 
                htmlFor="document" 
                className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300/50 border-dashed rounded-xl hover:border-indigo-300/70 transition-all duration-200 backdrop-blur-sm bg-white/30 cursor-pointer hover:bg-white/40"
              >
                <div className="space-y-1 text-center">
                  ...
                  <div className="flex text-sm text-gray-600 justify-center">
                    <span className="font-medium text-indigo-600 hover:text-indigo-500">Odaberite datoteke</span>
                    <span className="ml-1">ili ih prevucite ovdje</span>
                  </div>
            ...
            {(selectedFiles || []).length > 0 && (
              <div className="mt-4">
                ...
                <div className="max-h-40 overflow-y-auto border border-gray-200/50 rounded-xl p-2 bg-white/60 backdrop-blur-sm shadow-lg">
                  {selectedFiles.map((file, index) => (
                    <div key={index} className="flex items-center justify-between py-2 px-3 bg-gray-50/80 backdrop-blur-sm rounded-lg mb-1 hover:bg-indigo-50/80 transition-all duration-200 border border-gray-200/30">
                      ...
                      <button 
                        ...
                        className="text-red-500 hover:text-red-700 p-1 rounded-full hover:bg-red-50/80 backdrop-blur-sm transition-all duration-200"
                      >
      ...
      <div className="bg-gradient-to-r from-gray-50/80 to-gray-100/80 backdrop-blur-sm px-6 py-4 flex items-center justify-end space-x-3 border-t border-white/30 rounded-b-2xl">
        <button
          type="button"
          onClick={onCancel}
          className="inline-flex items-center px-4 py-2 border border-gray-300/50 shadow-sm text-sm font-medium rounded-xl text-gray-700 bg-white/80 backdrop-blur-sm hover:bg-gray-50/80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200"
        >
          Odustani
        </button>
        <button
          ...
          className={`inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-xl text-white transition-all duration-200 backdrop-blur-sm ${
            isSubmitting

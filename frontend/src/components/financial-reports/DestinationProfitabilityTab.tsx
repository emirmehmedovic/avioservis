...
import financialReportsService, { DestinationProfitabilityItem } from '@/services/financialReportsService';

export function DestinationProfitabilityTab() {
  ...
  // Stanje za podatke
  const [reportData, setReportData] = useState<DestinationProfitabilityItem[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);

  // Definicija kolona za tabelu
  const columns: ColumnDef<DestinationProfitabilityItem>[] = [
    {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = await financialReportsService.getDestinationProfitabilityReport(params);
      setReportData(data);
    } catch (err) {
  ...
  const chartData = React.useMemo(() => {
    ...
    // Uzimamo top 10 destinacija sortirano po profitu
    const topItems = [...reportData]
      .sort((a, b) => b.profit - a.profit)
      .slice(0, 10);
    
    return {
      labels: topItems.map(item => item.destination),
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="border border-gray-200 bg-white shadow-sm">
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${summary.totalProfit >= 0 ? 'text-[#3B82F6]' : 'text-red-500'}`}>
                  {new Intl.NumberFormat('bs-BA', {
                    style: 'currency',
                    currency: 'BAM',
            ...
            <Card className="border border-gray-200 bg-white shadow-sm">
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${summary.averageMargin >= 0 ? 'text-[#3B82F6]' : 'text-red-500'}`}>
          ...
          <Card className="border border-gray-200 bg-white shadow-sm">
            <CardHeader className="border-b border-gray-200 bg-gray-50">
              <CardTitle className="text-lg text-gray-800">Top 10 destinacija po profitabilnosti</CardTitle>
            </CardHeader>
          ...
          <Card className="border border-gray-200 bg-white shadow-sm">
            <CardHeader className="border-b border-gray-200 bg-gray-50">
              <CardTitle className="text-lg text-gray-800">Profitabilnost po destinacijama - detalji</CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
              <DataTable
                columns={columns}
                ...
                filterPlaceholder="Pretraži po destinaciji..."
              />
            </CardContent>
          </Card>
        </>
